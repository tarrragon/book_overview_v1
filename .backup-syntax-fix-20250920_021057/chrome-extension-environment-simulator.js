...
class ChromeExtensionEnvironmentSimulator {
  ...
  async openPopupWindow (tabInfo = {}) {
    try {
      ...
      throw (() => { const error = new Error(`Failed to open popup window: ${error.message}`); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
  }
  ...
  async injectContentScript (pageContext = {}) {
    try {
      ...
      throw (() => { const error = new Error(`Failed to inject content script: ${error.message}`); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
  }
