  ...
  describe('D. 錯誤處理和恢復測試 (12個案例)', () => {
    ...
    test('35. 自動重試機制', async () => {
      ...
      const flakyNetwork = () => {
        attemptCount++
        if (attemptCount <= 2) {
          throw (() => { const error = new Error('TEMPORARY_NETWORK_ERROR'); error.code = ErrorCodes.NETWORK_ERROR; error.details = { category: 'testing' }; return error })()
        }
        return { success: true }
      }
      ...
      const maxRetryResult = await device.executeWithRetry(
        () => { throw (() => { const error = new Error('PERSISTENT_ERROR'); error.code = ErrorCodes.'E2E_PERSISTENT_ERROR'; error.details = { category: 'testing' }; return error })() },
        { maxRetries: 3 }
      )
