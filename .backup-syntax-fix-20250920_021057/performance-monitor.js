...
class PerformanceMonitor {
  ...
  startTimer (operationName) {
    if (this.timers.has(operationName)) {
      throw (() => { const error = new Error(`Timer ${operationName} already running`); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
  ...
  endTimer (operationName) {
    if (!this.timers.has(operationName)) {
      throw (() => { const error = new Error(`Timer ${operationName} not found`); error.code = ErrorCodes.NOT_FOUND_ERROR; error.details = { category: 'testing' }; return error })()
    }
  ...
  detectMemoryLeaks (baselineSnapshot, currentSnapshot) {
    if (!baselineSnapshot || !currentSnapshot) {
      throw (() => { const error = new Error('需要基準和當前記憶體快照進行比較'); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
