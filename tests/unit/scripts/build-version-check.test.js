...
describe('--skip-version-check flag 行為宣告', () => {
  // 1.0.0-W2-003 後 build 流程只在 prod mode 執行唯讀 validateVersionAlignment，
  // 不再自動呼叫 syncWorklogStatus（移除 worklog 改寫副作用）。--skip-version-check
  // 略過該唯讀斷言。純函式對 flag 不感知，由 build.js main flow 在呼叫前判斷
  // process.argv。syncWorklogStatus 純函式仍保留匯出供需要的流程使用。
