 ...
 *
 * TDD Phase 2 — 資料遷移 v1 → v2（Tag-based Book Model）測試
 * 這些測試對應尚未實作的遷移邏輯，預期全部 failing。
 *
 * 測試群組：
...
// 預期遷移函式的模組路徑（尚未實作）
// const { migrateV1ToV2, migrateReadingStatus, normalizeProgress } = require('src/data-management/migration/v1-to-v2')
...
describe('Schema Migration v1 → v2', () => {
  ...
  describe('遷移步驟 — 預設資料建立', () => {
    test('應備份原始資料至 migration_backup key', async () => {
      ...
      // await migrateV1ToV2(mockStorage, mockLogger)
      // expect(mockStorage._store.migration_backup).toBeDefined()
      // expect(mockStorage._store.migration_backup.readmoo_books).toHaveLength(1)
      expect(true).toBe(false)
    })
