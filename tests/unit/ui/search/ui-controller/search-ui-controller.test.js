...
describe('SearchUIController', () => {
  ...
  describe('1. Construction and Initialization', () => {
    ...
    test('應該拋出錯誤當缺少必要依賴', () => {
      try {
        // eslint-disable-next-line no-unused-vars
        const _controller = new SearchUIController({})
      ...
      try {
        // eslint-disable-next-line no-unused-vars
        const _controller = new SearchUIController({ eventBus: mockEventBus })
        // 變數賦值確保建構子結果被正確處理，測試錯誤條件
        throw new Error('應該拋出錯誤')
      } catch (error) {
        expect(error).toMatchObject({
      ...
      try {
        // eslint-disable-next-line no-unused-vars
        const _controller = new SearchUIController({ document: mockDocument })
        // 變數賦值確保建構子結果被正確處理，測試錯誤條件
        throw new Error('應該拋出錯誤')
      } catch (error) {
        expect(error).toMatchObject({
