...
describe('🖥️ Overview 頁面控制器測試 (TDD循環 #26)', () => {
    ...
    dom = new JSDOM(`
      ...
      <body>
        <div class="container">
          ...
          <!-- 搜尋區域 -->
          <input type="text" id="searchBox" placeholder="[CHECK] 搜尋書籍標題...">
          
          <!-- 操作按鈕區域 -->
          <div class="export-buttons">
            <button class="export-btn" id="exportCSVBtn">[STATS] 匯出 CSV</button>
            <button class="export-btn" id="exportJSONBtn">匯出 JSON</button>
            <button class="export-btn" id="importJSONBtn">匯入 JSON</button>
            <button class="export-btn" id="selectAllBtn">[OK] 選取全部</button>
  ...
  describe('Group G：UC-04 匯入持久化接線（0.19.0-W1-047.3）', () => {
    ...
    function setupController (importResult, writeResult) {
      ...
      // Group G TC-G3 使用 books: [] 覆蓋模式 fixture，若不 stub 會 hang 在 Modal B。
