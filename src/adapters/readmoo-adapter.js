...
class ReadmooAdapter {
  ...
  constructor (options = {}) {
    ...
    // 效能優化緩存
    this._urlValidationCache = new Map()
    this._coverIdCache = new Map()
   ...
   */
  validatePage (url) {
    return this._validateUrl(url, this.supportedUrls)
  }

  /**
   * 驗證並解析 URL（共用工具方法）
   * @param {string} url - 要驗證的 URL
   * @param {string[]} allowedDomains - 允許的域名列表
   * @returns {boolean} 是否為有效的 URL
   */
  _validateUrl (url, allowedDomains = []) {
    if (!url || typeof url !== 'string' || url.trim() === '') {
      return false
    }

    // 檢查快取
    const cacheKey = `${url.trim()}-${allowedDomains.join(',')}`
    if (this._urlValidationCache.has(cacheKey)) {
      return this._urlValidationCache.get(cacheKey)
    }
    ...
    try {
      ...
      if (!['http:', 'https:'].includes(urlObj.protocol)) {
        ...
        if (allowedDomains.length > 0) {
          result = allowedDomains.some(domain =>
    ...
    // 儲存結果到快取（但限制快取大小）
    if (this._urlValidationCache.size < 100) {
      this._urlValidationCache.set(cacheKey, result)
    }
  ...
  /**
   * 提取書籍 ID - 使用多層級策略確保識別穩定性
   *
   * 識別策略優先級：
   * 1. 主要策略：從封面圖片 URL 提取 ID (cover-*) - 最穩定
   * 2. 備用策略：基於書籍標題生成 ID (title-*) - 中等穩定性
   * 3. 最後備用：使用閱讀器連結ID (unstable-*) - 不穩定，會變更
   *
   * @param {Element} element - 書籍元素
   * @returns {string|null} 書籍 ID，包含來源前綴以指示穩定性
   */
  ...
  extractBookIdFromCover (element) {
    try {
      ...
      // 使用共用URL驗證方法
      if (!this._validateUrl(src)) {
        return null
      }

      // 檢查是否為Readmoo封面URL
      if (!this._isReadmooCoverUrl(src)) {
        return null
      }

      return this._extractIdFromReadmooCoverUrl(src)
    } catch (error) {
      return null
    }
  }
  ...
  /**
   * 從Readmoo封面URL提取ID
   * @param {string} url - Readmoo封面URL
   * @returns {string|null} 提取的ID
   */
  _extractIdFromReadmooCoverUrl (url) {
    // 檢查快取
    if (this._coverIdCache.has(url)) {
      return this._coverIdCache.get(url)
    }
    ...
    // 提取核心識別碼部分
    const coverMatch = url.match(/\/cover\/[a-z0-9]+\/([^_]+)_/)
    if (coverMatch) {
      result = `cover-${coverMatch[1]}` // 使用 cover- 前綴標識來源
    } else {
      // 備用解析方式
      const filenameMatch = url.match(/\/([^/]+)\.(jpg|png|jpeg)/i)
      if (filenameMatch) {
    ...
    // 儲存到快取（但限制快取大小）
    if (result && this._coverIdCache.size < 100) {
      this._coverIdCache.set(url, result)
    }
  ...
  reset () {
    ...
    // 清空快取
    this._urlValidationCache.clear()
    this._coverIdCache.clear()
  }
