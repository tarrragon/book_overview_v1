...
class LoadingOptimizer {
  ...
  preloadHighPriorityResources() {
    ...
    console.log(`📦 已排程預載入 ${highPriorityResources.length} 個高優先級資源`);
  }
  ...
  async loadOnDemand(resourceName) {
    console.log(`📦 按需載入資源: ${resourceName}`);
  ...
  async warmupCache(resourceNames) {
    console.log(`🔥 開始快取預熱: ${resourceNames.length} 個資源`);
  ...
  destroy() {
    ...
    console.log('🗑️ LoadingOptimizer 已銷毀');
  }
}
