...
### 基本資訊
- **用例ID**: UC-07  
- **用例名稱**: 跨裝置資料同步（QR 離線同步 + 未來雲端同步）
- **主要行為者**: 使用者（手動觸發 QR 同步）/ 系統（自動雲端同步，未來）
- **利益關係人**: 使用者（跨裝置存取書庫資料）
- **前置條件**: Extension 端已有提取的書庫資料；App 端已安裝
- **成功保證**: 書庫資料可在 Extension 與 App 間雙向同步

### 主要成功場景

#### 7A. QR 離線同步：Extension → App（v1.2.0，PROP-012）

1. **使用者觸發同步**
   - 使用者在 Extension Popup 點擊「同步到 App」按鈕
   - 系統從 `chrome.storage.local` 讀取書庫資料

2. **資料壓縮與 QR 編碼**
   - 書庫 JSON 加入 `sync_meta`（format_version 2.0、總書數、時間戳）
   - gzip 壓縮 → CRC32 校驗 → 切塊為 QR frames
   - Frame 格式：SPEC-009（`docs/spec/synchronization/SPEC-009-qr-frame-format.md`）

3. **QR 動畫輪播**
   - Extension Canvas 依序顯示 QR frames
   - 單幀（資料量小）時靜態顯示
   - 多幀時動畫輪播，顯示進度（如「共 5 幀，請以 App 掃描」）

4. **App 端掃描與解碼**
   - App 以相機掃描 QR 動畫，逐幀接收
   - 收齊所有 frames 後拼接 → CRC32 驗證 → gzip 解壓 → JSON 解析

5. **智慧合併**
   - 同 ID 書籍以 `updated_at` 較新者勝出
   - 新書直接新增；本機已有且無變更則跳過
   - 合併結果顯示摘要（新增/更新/未變更）

#### 7B. JSON 匯入：App → Extension（v1.2.0，PROP-012）

1. **使用者觸發匯入**
   - 使用者在 Extension Popup 點擊「匯入書庫」按鈕
   - 系統開啟檔案選擇對話框（accept `.json`）

2. **檔案解析與驗證**
   - 讀取 JSON → 偵測來源格式（canonical / app-legacy / v1 / v2）
   - 驗證 `sync_meta` 結構完整性
   - 空書籍陣列時回報錯誤

3. **防舊蓋新檢查**
   - 比較 `exported_at` 與本機 `last_imported_at`
   - 若匯入檔案較舊，以 `window.confirm` 警告使用者
   - 使用者確認後以 `skipStalenessCheck` 繼續匯入

4. **智慧合併與摘要**
   - 合併策略同 7A 步驟 5
   - Popup 顯示合併摘要（新增/更新/未變更）
   - 匯入失敗時顯示錯誤訊息
...
> 本 UC 是 UC-01 / UC-02 的**自動化對應流程**。差異：UC-01/02 假設使用者已在書庫頁且手動觸發單次提取；本 UC 提供「一鍵完成從任意頁面到完整書目蒐集」的全自動化路徑。

### 主要成功場景

#### 9A. 一鍵導航

1. **起點判定** — 使用者在任意頁面點擊 Extension 圖示開啟 Popup，Popup 偵測當前 tab URL 判斷是否已在書庫頁
2. **程式化跳轉** — 系統透過 `chrome.tabs.update` 導向書庫頁，於 `chrome.storage.session` 寫入 `autoStartExtraction: true` 旗標
3. **頁面載入與旗標傳遞** — content script 讀取旗標，存在則自動進入 9B 流程

#### 9B. 自動滾動蒐集

1. **DOM Ready 等待** — MutationObserver + timeout 雙保險確認書籍容器已出現
2. **滾動演算法** — `window.scrollTo` + `scrollHeight` 變化偵測，連續 2 次無變化判定已到底，safetyBreak > 100 強制終止
3. **資料擷取與去重** — 以 `bookId` 為 key 維護 Set 去重，進度面板更新
4. **完成寫入** — 書目寫入 `chrome.storage.local`，清除旗標

### 替代流程

- **9C. 使用者未登入** — 保留旗標，登入後自動繼續
- **9D. 使用者中斷** — 已蒐集資料標記 `status: 'partial'` 保留
- **9E. 網路延遲** — 漸進退避重試，3 次失敗標記 partial
- **9F. 儲存容量接近上限** — 分級警告（80% INFO / 90% WARN / 95% BLOCK）

> **完整場景細節**: `docs/usecases/UC-09-auto-full-library-extraction.md`
...
- **前置條件**:
  - Extension 已安裝且已授予目標書城的 host 權限
  - 使用者已在目標書城登入
- **成功保證**: Extension 自動偵測當前書城並載入正確的 adapter，無需使用者手動選擇

> 本 UC 是多書城架構的入口流程（v1.5.0+）。描述 Extension 如何自動偵測使用者所在的書城網站、載入對應的 adapter、以及在多書城間切換時的 UX 行為。

### 主要成功場景

#### 10A. 書城自動偵測

1. **URL 偵測** — Content Script 呼叫 `PlatformRegistry.detect(currentUrl)` 比對 hostname
2. **Adapter 載入** — 依 PlatformConfig 的 `adapterFactory` 建立書城專屬 adapter 實例
3. **Popup 書城識別** — Popup 顯示當前偵測到的書城名稱和圖示

#### 10B. 首次造訪新書城

1. **新書城偵測** — 系統檢查 `{platformId}_books` key 不存在
2. **引導提示** — Popup 顯示引導訊息，使用者點擊提取建立該書城 storage key
3. **Storage 初始化** — 各書城 storage key 並存且完全隔離

#### 10C. 書城間分頁切換

1. **多分頁場景** — 切換分頁時 Popup 自動偵測當前活動分頁的書城
2. **Popup 狀態更新** — 即時反映當前分頁書城的書庫統計
3. **提取歸屬正確** — 提取結果歸屬當前分頁對應的 `{platformId}_books` key

### 替代流程

- **10D. 不支援的書城** — `PlatformRegistry.detect()` 回傳 null，Popup 顯示「此網站暫不支援」
- **10E. 書城頁面但非書庫頁** — Popup 提示導航到書庫頁面
- **10F. Adapter 載入失敗** — 僅該書城不可用，其他書城不受影響

> **完整場景細節**: `docs/usecases/UC-10-multi-platform-detection.md`
...
> 本 UC 描述多書城資料共存時的合併檢視、按書城篩選、跨書城去重情境。v2.0.0 新增，依賴 UC-10 和 SPEC-STORAGE-ISOLATION。

### 主要成功場景

#### 11A. 合併書庫檢視

1. **載入合併資料** — `loadAllPlatformBooks()` 讀取所有 `{platformId}_books` key，合併為單一清單
2. **書城來源標示** — 書籍旁顯示書城來源圖示，動態依 PlatformRegistry 載入
3. **統計資訊** — 頁面頂部顯示合併統計

#### 11B. 按書城篩選

1. **篩選操作** — 書城篩選下拉選單動態列出所有已有資料的書城
2. **組合篩選** — 書城篩選可與既有篩選（閱讀狀態、tag、關鍵字搜尋）組合

#### 11C. 跨書城去重

1. **重複偵測** — 以 ISBN 比對，合併檢視中以單一卡片呈現多來源
2. **合併卡片** — 卡片顯示多書城來源標籤，閱讀進度取各書城最高值
3. **去重範圍** — 僅在合併檢視時執行，不修改各書城原始 storage

### 替代流程

- **11D. 僅單一書城有資料** — 合併檢視退化為單書城檢視
- **11E. 合併讀取部分失敗** — 成功讀取的書城正常顯示，失敗的書城顯示警告
- **11F. ISBN 缺失導致去重不完整** — 無 ISBN 書籍各自獨立顯示

> **完整場景細節**: `docs/usecases/UC-11-cross-platform-merged-view.md`
...
> 本 UC 描述單一書城的資料管理操作：書城級資料清除、書城級匯出、書城啟用/停用。v2.0.0 新增，依賴 SPEC-STORAGE-ISOLATION。

### 主要成功場景

#### 12A. 書城級資料清除

1. **選擇目標書城** — 管理介面書城清單中選擇，顯示資料摘要
2. **確認清除** — 確認對話框明示僅影響該書城，其他書城不受影響
3. **執行清除** — 刪除 `{platformId}_books` key，合併檢視即時更新
4. **清除後狀態** — 下次造訪該書城並提取時重新建立 storage key

#### 12B. 書城級匯出

1. **選擇匯出範圍** — 「全部書城」或「特定書城」
2. **生成匯出檔案** — 僅讀取該書城 key，檔案命名含書城標識
3. **匯出格式** — 遵循 book-interchange-v1 canonical v3 規範

#### 12C. 書城啟用/停用

1. **停用書城** — 保留資料但排除於合併檢視，Content Script 不啟動
2. **重新啟用** — 恢復合併檢視與偵測功能，既有資料完整保留

### 替代流程

- **12D. 清除最後一個書城** — Overview 顯示空書庫狀態，觸發 UC-05 的引導流程
- **12E. 匯出時 storage 讀取失敗** — 顯示錯誤，可重試或選擇其他書城
- **12F. 停用書城時有進行中的提取** — 進行中的提取完成後寫入，停用在下次合併檢視生效

> **完整場景細節**: `docs/usecases/UC-12-single-platform-management.md`
