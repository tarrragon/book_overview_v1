...
- 評估方法：team-design-evaluation 五 lens 判讀協議（發射管制隊視角）
...
| Lens               | 現狀                                                            | 判讀                                                     | 行動                          |
| ------------------ | --------------------------------------------------------------- | -------------------------------------------------------- | ----------------------------- |
| 常設 vs 編成       | registry 常設層 + dispatch log 歷史、rosemary（主 context）為唯一調度點 | 編成可事後重建（log 過濾）、非一等公民記錄；解編有 hook 自動化 | 保留                          |
...
2. **名冊膨脹**：registry 30 名（含 2 deprecated）、dispatch log 約 1000 筆中 10 名零派發；impeccable 有實質工作卻未登記；mint 的工作是 `dart fix` / `dart format` 的 agent 包裝。
...
4. **文字介面脆弱殘留**：兩專案根目錄有 hook 提示文字誤建的目錄（「（待恢復 handoff）…」）。
