...
class SyncErrorHandler {
  ...
  async handleNetworkError(error, context = {}) {
    const errorInfo = {
      ...
      originalError: error,
      context,
      suggestions: ['檢查網路連接狀態', '稍後重試同步操作', '使用離線備份功能'],
      recovery: {
  ...
  async executeWithRetry(operation, config = {}) {
    ...
    while (attemptCount <= finalConfig.maxRetries) {
      try {
        ...
        if (attemptCount > 1) {
          // 指數退避延遲
          const delay =
            finalConfig.baseDelay * Math.pow(finalConfig.backoffMultiplier, attemptCount - 2)
    ...
    return {
      ...
      manualOptions: ['contact_support', 'check_network_connection', 'try_alternative_sync_method']
    }
  }
  ...
  isRetryableError(error) {
    const retryableTypes = ['NETWORK_ERROR', 'TEMPORARY_STORAGE_ERROR', 'TIMEOUT_ERROR']

    return retryableTypes.includes(error.type) || error.message.includes('TEMPORARY')
  }
}
...
test('應該在記憶體限制下正常工作', async () => {
  ...
  const result = await processor.processBooksInBatches(generateTestBooks(5000), mockProcessor, null)
