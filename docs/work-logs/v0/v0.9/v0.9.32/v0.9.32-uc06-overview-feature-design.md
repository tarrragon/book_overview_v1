...
const testBookData = {
  ...
  boundary: [
    // 極長書名
    {
      id: 'long-title-test',
      title:
        '這是一個非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常非常長的書名用來測試系統對極長書名的處理能力',
...
describe.each([
  ...
  beforeEach(() => {
    ...
    isolatedController = new OverviewPageController(isolatedMocks.eventBus, isolatedMocks.document)
  })
...
class EventHandlerMock {
  ...
  // 工具方法Mock
  canHandle(event) {
    return this.getSupportedEvents().includes(event.type)
  }
  setEnabled(enabled) {
