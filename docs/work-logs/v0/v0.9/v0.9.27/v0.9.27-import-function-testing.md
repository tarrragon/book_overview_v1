...
describe('📄 檔案載入基本功能', () => {
  ...
  test('應該能夠載入包含books屬性的JSON檔案', async () => {
    ...
    const fileContent = JSON.stringify({
      books: [{ id: 'book-1', title: '測試書籍', cover: 'http://example.com/cover.jpg' }],
...
describe('❌ 檔案格式錯誤處理', () => {
  test('應該處理無效的JSON格式', async () => {
    ...
    // Then: 驗證錯誤處理
    expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('JSON 檔案格式不正確'))
    expect(controller.currentBooks).toHaveLength(0)
  })
  ...
  test('應該處理非陣列格式的JSON', async () => {
    ...
    // Then: 驗證錯誤處理
    expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('應該包含一個陣列'))
  })
  ...
  test('應該處理FileReader讀取錯誤', async () => {
    ...
    // Then: 驗證錯誤處理
    expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('讀取檔案時發生錯誤'))
  })
})
```
...
- `應該能夠載入有效的JSON檔案`: 增強handleFileLoad和\_handleFileContent處理有效JSON陣列
...
2. **強化\_handleFileContent方法**: JSON解析錯誤處理、字符編碼支援
3. **完善\_extractBooksFromData方法**: 多格式支援、錯誤訊息改善
4. **新增\_isValidBook方法**: 基本資料驗證邏輯
