...
比較指標: -UI回應時間變化率 - 資料處理速度變化率 - 記憶體使用效率變化 - 錯誤率變化趨勢
```
...
// 異常處理策略
異常類型A: UI回應延遲 - 記錄詳細的執行時間分析 - 檢查DOM複雜度和事件處理數量 - 觸發UI優化建議
...
異常類型C: 處理超時 - 記錄當前處理進度和卡住的操作 - 提供使用者中止操作的選項 - 實施分段處理降級策略
```
...
```javascript
// 效能降級策略
降級觸發條件: -記憶體使用超過90MB - (處理時間超過預期50 % -併發操作數量超過系統限制)

降級措施: -減少同時處理的書籍數量 - 暫停背景處理任務 - 簡化UI動畫和視覺效果 - 增加處理進度的更新頻率
```
...
```javascript
錯誤場景: -網路連線中斷期間的操作嘗試 - DNS解析失敗時的重試機制 - 慢速網路下的超時處理
  ...
  getPerformanceMetrics() {
    return {
      averageStorageReadTime: this.calculateAverage(
        this.storageOperationTimes.filter((op) => op.operation === 'get').map((op) => op.duration)
      ),
      averageStorageWriteTime: this.calculateAverage(
        this.storageOperationTimes.filter((op) => op.operation === 'set').map((op) => op.duration)
      ),
...
class MockTestDataGenerator {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (settings.fieldsCount > 10) {
        book.metadata = {
          ...
          categories: Array.from({ length: 3 }, () => this.generateRandomString(15))
        }
      }
  ...
  generateJSONFile(targetSize, format = 'books') {
    ...
    while (currentSize < targetSizeBytes * 0.95) {
      // 95%目標大小
...
class PerformanceTestDataGenerator {
  ...
  generateJSONFile(targetSize, options = {}) {
    const { format = 'standard', compression = 'none', includeMetadata = true } = options
    ...
    while (currentSize < targetSizeBytes * 0.95 && books.length < estimatedBookCount * 1.2) {
      const book = this.generateSingleBook(
        bookIndex++,
  ...
  generateTags(count) {
    ...
    return Array.from(new Set(Array.from({ length: count }, () => this.selectRandom(allTags))))
  }
...
describe('基礎效能測試 - UI回應時間', () => {
  ...
  describe('A1. UI回應時間基準測試', () => {
    test('A1-1: Popup開啟效能測試 - 應在200ms內完成', async () => {
      ...
      // When: 使用者點擊Extension圖示開啟Popup
      const { result, timing } = await performanceMonitor.measureAsync('popup-open', async () => {
  ...
  describe('A2. 資料處理效能基準測試', () => {
    test('A2-1: 小量書籍提取效能測試 - 10本書籍應在1秒內完成', async () => {
      ...
      console.log(
        `✅ 10本書籍提取時間: ${timing.duration.toFixed(2)}ms, 成功率: ${(result.successRate * 100).toFixed(1)}%`
      )
    })
    ...
    test('A2-2: 中量書籍提取效能測試 - 100本書籍應在8秒內完成', async () => {
      ...
      console.log(
        `✅ 100本書籍提取時間: ${timing.duration.toFixed(2)}ms, 成功率: ${(result.successRate * 100).toFixed(1)}%`
      )
    })
    ...
    test('A2-3: JSON檔案解析效能測試 - 2MB檔案應在4秒內完成', async () => {
      ...
      console.log(
        `✅ JSON解析速度: ${processingSpeedMBs.toFixed(2)}MB/s, 記憶體增長: ${memoryGrowthMB.toFixed(2)}MB`
      )
    })
  })
...
2. **記憶體測試通過策略**:
   ...
   async function validateMemoryUsage(beforeSnapshot, afterSnapshot, maxGrowthMB) {
     const growthMB =
       (afterSnapshot.memory.heapUsed - beforeSnapshot.memory.heapUsed) / (1024 * 1024)
     ...
     if (growthMB > maxGrowthMB) {
       ...
       if (global.gc) {
         ...
         const finalGrowthMB =
           (cleanupSnapshot.memory.heapUsed - beforeSnapshot.memory.heapUsed) / (1024 * 1024)
...
3. **資料處理測試通過策略**:
   ...
   async function validateDataProcessingPerformance(dataSize, expectedTime) {
     ...
     for (let batch = 0; batch < batches; batch++) {
       ...
       const { timing } = await performanceMonitor.measureAsync(`batch-${batch}`, () =>
   ...
   const performanceCodeQualityChecks = {
     ...
     checkTestTimeouts: (testFiles) => {
       testFiles.forEach((file) => {
         const hasTimeout =
           file.content.includes('timeout') || file.content.includes('jest.setTimeout')
