...
class PlatformAdapterInterface {
  // 頁面檢測方法
  async getPageType() { throw new Error('Must implement') }
  async isExtractablePage() { throw new Error('Must implement') }
  async checkPageReady() { throw new Error('Must implement') }
  
  // 元素查找方法
  getBookElements() { throw new Error('Must implement') }
  getBookCount() { throw new Error('Must implement') }
  
  // 資料提取方法
  parseBookElement(element) { throw new Error('Must implement') }
  async extractAllBooks() { throw new Error('Must implement') }
  
  // 工具方法
  sanitizeData(data) { throw new Error('Must implement') }
  getStats() { throw new Error('Must implement') }
}
```
