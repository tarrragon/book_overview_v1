...
class ErrorCoordinator {
  constructor(
    ...
    options?: ErrorCoordinatorOptions
  );
}
  ...
  private async _processEventQueue(): Promise<void> {
    ...
    while (!this._eventQueue.isEmpty()) {
      ...
      await Promise.allSettled(
        batch.map(event => this._handleSingleEvent(event))
      );
...
class ErrorCoordinator {
  ...
  private _determineHealthStatus(stats: ErrorStatistics): HealthStatusType {
    ...
    if (errorRate > 0.1 || memoryUsage > 100 * 1024 * 1024) { // 10% 錯誤率或 100MB 記憶體
      return HealthStatusType.UNHEALTHY;
    } else if (errorRate > 0.05 || memoryUsage > 50 * 1024 * 1024) { // 5% 錯誤率或 50MB 記憶體
...
*本規範文件將作為實作的權威參考，任何變更都需要更新此文件並記錄在工作日誌中。*