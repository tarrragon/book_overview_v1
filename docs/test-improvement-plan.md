  ...
  describe('✅ 正常匯入流程', () => {
    test('應該能成功載入有效的JSON檔案', async () => {
      ...
      const mockFile = new File(
        [JSON.stringify(validBookData)], 
        'readmoo-books-2025-08-22.json',
    ...
    test('應該在覆蓋模式下清空現有資料', async () => {
      ...
      global.booksData.push(
        { id: "existing-1", title: "既有書籍" }
      )
      
      const newBookData = [
        { id: "new-1", title: "新書籍", source: "readmoo" }
      ]
    ...
    test('應該在合併模式下保留現有資料並去重', async () => {
      // 準備現有資料
      global.booksData.push(
        { id: "cover-123", title: "既有書籍", progress: 30 }
      )
...
describe('📋 Overview頁面功能測試', () => {
  ...
  // 輔助函數（需要實際實現）
  function renderTable() { /* 實際的渲染邏輯 */ }
  function createBookRow(book) { /* 建立書籍行的邏輯 */ }
  function searchBooks(books, term) { /* 搜尋邏輯 */ }
  // ... 其他輔助函數
})
```
