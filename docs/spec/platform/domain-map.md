...
|---|---|
| Platform Registry | 註冊的 platformId 唯一；已註冊平台可查詢；未註冊平台查詢回傳 null |
| Platform Detection | readmoo.com 識別為 READMOO；books.com.tw 識別為 BOOKS_COM_TW；kobo.com 識別為 KOBO；未知域名回傳 null |
| Platform Switcher | 切換至已註冊平台成功；切換至未註冊平台失敗 |
| Adapter Factory | 工廠產出的 adapter 實作 PlatformAdapterInterface；資源池化不重複建立相同平台的 adapter |
