...
|---|---|
| QR Frame Codec | magic 恆為 0x5152；version 恆為 0x01；frame_index 0-based 連續；所有 frame 的 total_frames 和 crc32 相同；拼接後長度等於 total_size |
| Sync Meta | format_version 必填；source 必為 'extension'；total_books >= 0 |
| Merge Rules | 正常遷移產出 reader-{privacyBookId} 格式 ID；同 ID 多筆合併後 tagIds 為並集 |
