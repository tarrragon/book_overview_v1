...
# 視為「終止狀態」的 status 值（對應 AC 2：completed/closed）
TERMINAL_STATUSES: Set[str] = {"completed", "closed"}


def _read_children_from_file(ticket_file: Path, logger) -> List[str]:
    """直接從 Ticket 檔案內容抽取 children 清單。

    背景：hook_utils 的內建 YAML frontmatter parser 無法解析
    頂層 YAML 列表（`children:\n- id1\n- id2`），會將 children
    值解析為空字串。為避免依賴不穩定的解析結果，本函式直接從
    frontmatter 原始文字中解析 children 欄位。

    回傳：children ID 清單（順序保留）。找不到或為空時回傳 []。
    """
    try:
    ...
    # 逐行掃描：找到 "children:" 行之後，累積直到下一個頂層鍵
    children: List[str] = []
    in_children = False
    for raw_line in frontmatter_text.split("\n"):
        ...
        if not in_children:
            # 偵測 children: 起始行
            m = re.match(r"^children:\s*(.*)$", line)
            if m:
                inline = m.group(1).strip()
                if inline and inline != "[]":
                    # 內聯 flow-style 列表 `children: [a, b]`
                    flow = re.match(r"^\[(.*)\]$", inline)
                    if flow:
                        items = [s.strip() for s in flow.group(1).split(",")]
                        children.extend([i for i in items if i])
                        return children
                in_children = True
            continue
        ...
        # 判斷是否仍在 children 區塊：允許 "  - id" 或 "- id"
        if stripped.startswith("- "):
            child_id = stripped[2:].strip()
            if child_id:
                children.append(child_id)
            continue

        # 其他非列表行 → 視為離開 children 區塊
...
def _extract_children_robust(
    ...
    """綜合解析 children：先嘗試 frontmatter dict，fallback 檔案原始文字。"""
    # 優先：使用既有 extractor（處理 list 型 frontmatter）
    children = extract_children_from_frontmatter(frontmatter, logger)
    if children:
        return children

    # Fallback：直接從原始檔案抽取（應對 hook_utils parser 對頂層列表解析失敗）
    return _read_children_from_file(ticket_file, logger)
