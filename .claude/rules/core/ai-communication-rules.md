...
**Why**：估時是可量化的誘餌，容易取代「是否應該做」的本質判斷。AI 輔助開發中估時準確度低（token 預算波動大、推理深度難預測），用估時當決策依據等於用雜訊當訊號。估時框架還會讓代理人傾向「討好式選擇快的路徑」，遮蔽正確但慢的解法（規則 5 §5.6 機制 4 反諂媚設計的變體）。
...
**邊界說明**：本規則禁止的是用估時當「決策依據」，不是禁止估時本身。估時仍可作為（1）資源規劃輸入；（2）Wave 容量檢查（容量定義見下方 hotpath 表）；（3）向用戶報告進度。關鍵是估時不進入「做 vs 不做」「做 A vs 做 B」的決策邏輯。
