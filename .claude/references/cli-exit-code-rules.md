...
**Consequence**：若未過此清單就放行 Phase 1，後續 Hook 串接時必再發現混淆，需回頭修規格與既有呼叫端（如 W15-028 已盤點 4 個違規檔案）。
...
**修復方向**：`report.overall_passed=False` → `return 2`；except 兩條維持 `return 1`。後續測試應加 return 2 與 return 1 各一個斷言案例（W15-028 acceptance 第 5 項）。
...
**Action**：修復 PC-096 違規點時（W15-028.2 等後續 IMP），業務拒絕路徑應同步檢查是否已輸出 `format_error(ErrorEnvelope)`，若無則一併補上；exit code 與訊息為同一 commit 的兩面，禁止分階段釋出。
...
**Version**: 1.0.0
**Source**: PC-096（W10-017.1 Phase 1 多視角審查 thyme 視角第 1 條結構性發現）+ W15-028 既有 CLI 稽核
