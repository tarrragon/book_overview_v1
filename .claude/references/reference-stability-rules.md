...
| Version footer `— W-xxx 落地` | 紀念型（對讀者無操作價值，sync 後死編號） | 禁止，改「歷史 X.Y 版見 git log」 |

**框架文件 footer 來源標注的處理**：框架文件 footer 的 `**Source**: W-xxx` / `Version: — W-xxx 落地` 不適用規則 7 來源標注例外。規則 7 例外限 `docs/`（不 sync）；框架文件 footer 在 sync 範圍且對讀者無操作價值（讀者不需知道哪個 ticket 修訂即可使用規則），維持 PC-098 立場改用「歷史 X.Y 版見 git log」。
...
**Version**: 1.1.0 — 規則 8 新增「引用性質判準」章節（依賴型禁止 vs 歷史錨點 / 設計脈絡型允許，操作判準＝移除 ticket ID 後可否理解），對齊 PC-093 `history` 豁免 + comment-writing 方法論 + 規則 7 來源標注例外，消除三方衝突。歷史 1.0 版見 git log。
