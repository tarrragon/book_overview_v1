...
**結論**：stale base 是 cc runtime worktree 的系統性行為，**每次** isolation:worktree 派發都可能遇到，落差大小取決於派發前主 repo 的 commit 活躍度。
...
理由：A 只能縮小初始落差，無法防止派發後的 main 新增 commit；C 在 cc runtime 閉源環境下難以可靠偵測 worktree 建立時機。B 覆蓋範圍最完整（初始落差 + 後續落差），agent 端可自律執行，成本為每次 prompt 多一行 git merge 指引。A1（PM 派發前 commit gate）作為輔助防護與 B 並用（見「與派發前 commit gate 的關係」）。
