...
### 1.7 最小變更紀律（Surgical Changes，編輯既有程式碼時強制）

> **觸發時機**：修改既有檔案以完成某項任務（修 bug、加功能、調整行為）時。與 1.2.4（未使用程式碼處理）的差異：1.2.4 處理「發現既有死碼該不該刪」，本節處理「本次改動的邊界該畫在哪」。

**核心原則**：每一行改動都應能直接對應到被交付的需求。改動範圍越小，引入回歸的面積越小、review 成本越低、git blame 越乾淨。

**強制檢查清單**（提交前逐項確認）：

| 步驟 | 動作 | 驗證方式 |
|------|------|---------|
| 1 | 逐行檢視 diff，問「這行改動對應需求的哪一項？」 | 無法回答者還原 |
| 2 | 確認未觸碰本次任務範圍外的程式碼 | diff 檔案清單對照任務目標 |
| 3 | 確認未做無關的重新格式化（縮排、引號風格、import 排序） | 格式差異與行為差異分離檢視 |
| 4 | 若清理死碼，確認該死碼是本次改動所造成 | 對照改動前狀態 |

**四條禁令**：

| 禁止行為 | Why（為何禁） | Consequence（違反後果） |
|---------|-------------|----------------------|
| 動未被要求的碼（順手改鄰近函式的命名、typo、風格） | 改動與任務無因果關係 | 擴大回歸面積、review 者需分辨哪些是任務哪些是順手 |
| 重新格式化未被要求格式化的檔案（reformat、改縮排、重排 import） | 格式改動淹沒真實行為改動 | 巨大 diff 遮蔽實際變更，review 幾乎無法進行 |
| 清理非自己造成的既有死碼 | 屬 1.2.4 / Phase 4 範圍，需獨立評估設計意圖 | 既有死碼可能是未實現的設計意圖，順手刪會讓需求消失 |
| 修改風格偏好（把 OOP 改成函式式、加分號、改 var/let） | 個人偏好凌駕檔案一致性 | 檔案內風格漂移，後續維護者認知負擔上升 |

**風格對齊要求**：新增的程式碼必須匹配所在檔案的既有風格（引號、命名慣例、有無分號、縮排），檔案內一致性優先於個人偏好。

**Action（越界時如何處理）**：修 bug 時發現鄰近有其他問題（命名不佳、typo、死碼、可重構點），**不當下順手改**——建 Ticket 追蹤（quality-baseline 規則 5），保持本次 diff 純粹。若一個修復開始級聯（改 A 觸發改 B 觸發改 C），停下來，這是範圍失控訊號（對照 1.2.6 共用函式修復範圍防護），先與派發者確認範圍再繼續。

**核心教訓**：「趁著我在這裡順便……」是 diff 膨脹的頭號來源。本次任務範圍外的每個改善都是一個獨立的 Ticket，不是一個順手的 commit。
...
- [ ] diff 每行可對應需求，無順手改動 / 無關 reformat / 越界死碼清理（1.7）
...
**Version**: 1.7.0 — 新增 §1.7「最小變更紀律（Surgical Changes）」（1.5.0-W5-009.1 落地，源自外部 CLAUDE.md 範例第 4/10 章）：四條禁令（越界改動 / 無關 reformat / 越界死碼清理 / 風格偏好凌駕）+ 三明示 + 風格對齊要求 + 級聯範圍失控訊號；§2 結構檢查清單同步補項
