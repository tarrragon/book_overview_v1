...
def _use_real_repo_root(real_repo_root):
    ...
    W1-050：覆蓋 autouse `_isolate_project_root` 的空 tmp 隔離；測試走
...
def test_missing_required_fields_listed_in_single_error():
...
def test_why_only_missing_still_reported_via_checklist():
...
def test_how_flag_gives_friendly_hint_with_value(capsys):
...
def test_how_flag_gives_friendly_hint_without_value(capsys):
...
def test_full_flags_unaffected_by_how_trap():
