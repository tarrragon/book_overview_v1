...
- `mutation-observer-scope.md` — MutationObserver 範圍與觸發頻率
- `mutationobserver-over-polling.md` — setTimeout 輪詢換 MutationObserver
- `reactive-listener-frequency-management.md` — Reactive 監聽器的效能 audit
- `runtime-iteration-and-regex-cost.md` — Runtime 計算成本：每筆迭代與正則
- `layout-reflow-measurement.md` — Layout reflow / repaint 的可量化評估
- `lazy-loading-and-critical-path.md` — 資源載入時序：lazy chunk 與 critical path
