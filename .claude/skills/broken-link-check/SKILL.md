...
排除：`http(s)://`（外部 URL）、`#section`（錨點）、預設三旋鈕涵蓋的程式碼區塊 / 備份目錄 / placeholder 範例。
...
**Source**: broken links 後置預防機制；1.0.0-W8-030.1 改路由至 scan_links.py 確定性 CLI 作權威 gate，手動流程降級為非權威 fallback
