...
```text
[ERROR] timestamp=... level=error component=... event=... error_type=... error_message=... <業務識別> <correlation_id>
```

- `error_type`：錯誤分類（`validation_error` / `timeout` / `permission_denied` 等）
- `error_message`：人類可讀的錯誤說明（不含敏感資料）
- 業務識別（至少一個）：`order_id` / `user_id` / `resource_id` 等
- Correlation ID（若有跨元件）
- 錯誤瞬間的關鍵狀態

#### Warn（3 項必填）

```text
[WARN] timestamp=... level=warn component=... event=... warn_type=... <業務識別>
```

- `warn_type`：警告分類（`degraded_mode` / `retry_succeeded` / `quota_approaching` 等）
- 業務識別
- 恢復方式（若有）：`retry_count=2` / `fallback=cache`

#### Info（1 項必填）

```text
[INFO] timestamp=... level=info component=... event=... <業務識別>
```
...
```text
[DEBUG] timestamp=... level=debug component=... event=... <狀態欄位>
```

- 允許記錄內部狀態、分支決策、變數值
- 必須能在生產環境關閉（透過 log level 配置）
...
```text
錯誤：[ERROR] Order 123 for user 42 failed because of insufficient funds at stripe
問題：grep "order_id=123" 找不到這條 log
正確：[ERROR] event=order.failed order_id=123 user_id=42 error_type=payment_declined error_message=insufficient_funds gateway=stripe
```

### 反模式 2：Severity 用來表達「問題嚴重度」而非「應否叫醒 on-call」

```text
錯誤：[ERROR] User entered wrong password
問題：使用者輸入錯誤不該觸發 on-call 警報
正確：[INFO] event=user.login_failed user_id=42 reason=wrong_password
```

### 反模式 3：Log 訊息隨程式碼位置而非業務事件命名

```text
錯誤：[DEBUG] Entering orderController.create() at line 142
問題：重構時訊息立刻失效；讀者不知道業務含義
正確：[DEBUG] event=order.creation.input_validating component=order-controller order_id=pending
```

### 反模式 4：迴圈中每次迭代都寫 info log
...
問題：污染 log、掩蓋真實事件
正確：
[INFO] event=batch.started job_id=xyz total=1000
[INFO] event=batch.completed job_id=xyz processed=998 failed=2 duration_ms=5432
```

### 反模式 5：錯誤 log 只有 error_message 沒有業務識別

```text
錯誤：[ERROR] Database connection timeout
問題：哪個查詢？哪個使用者的請求？無法定位
正確：[ERROR] event=db.query_timeout component=order-service request_id=abc123 query=select_orders timeout_ms=5000
```

### 反模式 6：混用時間格式或時區

```text
錯誤：時而 2026-04-16 12:00:00 時而 Apr 16 12:00:00
問題：跨系統聚合時無法排序、跨時區除錯困難
正確：統一 ISO 8601 + UTC（2026-04-16T12:00:00Z）
```

### 反模式 7：把 stack trace 塞進 log 訊息本體

```text
錯誤：[ERROR] Failed: at func.a (line 12), at func.b (line 34), at func.c (line 56)...
問題：訊息被稀釋、工具解析困難
正確：
[ERROR] event=order.failed error_type=null_pointer stack_trace_id=st_abc123
（stack trace 另存，或作為獨立 JSON 欄位）
```

### 反模式 8：使用主觀形容詞

```text
錯誤：[WARN] Query is slow
問題：「慢」沒有量化，無法監控
正確：[WARN] event=db.query_slow duration_ms=3200 threshold_ms=1000 query_name=select_user_orders
```
...
## 相關原則（在本 Skill 其他 reference 中）

- 原子化的通用判斷標準 → SKILL.md 五大原則速查
- 索引建立的通用技巧 → SKILL.md 五大原則速查
- 意圖顯性的通用寫作技巧 → SKILL.md 五大原則速查
- 可查詢性的通用設計 → SKILL.md 五大原則速查
- 欄位設計的通用精神 → SKILL.md 五大原則速查

本文件聚焦五大原則**在 log 情境的具體應用**，通用說明請回到 SKILL.md。
