...
**脈絡提示**：若計時值是 mock 回傳的固定數字，斷言該固定值是確定性的，不屬環境依賴 flaky。識別方式：追溯計時值來源，確認是否經過 `performance.now()` 的真實差值計算。
...
**Version**: 1.1.0 — F1 類型編號對齊 SKILL.md（5-8）、F6 移除 Node.js 後端服務、F7 補類型 9 斷言過度集中 section（Source: W1-027 第 1 輪審查）
