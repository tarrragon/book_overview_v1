...
| 問題 | 判斷標準 | 「否」時對應類型 |
|------|---------|----------------|
| 確定性 | 同一程式碼在任何執行環境下，pass/fail 結果一致？ | 類型 1–3（環境依賴） |
| 聚焦性 | 此測試案例只驗證一個行為面向？ | 類型 9（過度集中） |
...
- **環境依賴 flaky 族**（類型 1–3）：斷言結果受執行環境影響，核心問題是「確定性」
- **斷言設計族**（類型 4–9）：斷言內容設計不當，核心問題是「驗什麼」

兩軸正交：一個斷言可能同時落入兩族（例如高精度浮點斷言既有環境依賴問題，也可能有設計問題）。
...
**Version**: 1.0.0 — 初始建立（Source: W1-017 + W1-018 實證 4 類、W1-024 推導 5 類、W1-024 四視角審查結論）
