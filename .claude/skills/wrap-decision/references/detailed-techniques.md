...
以「Hook 錯誤處理」類的除錯場景為例：

| 層級 | 搜尋 | 結果 |
|------|------|------|
| 0. 身邊 | 本專案既有模組（例如追蹤器/派發器） | 已有 API 但沒接線 |
| 1. 同層 | 同類元件的清理機制 | 類似功能的 Hook 已有方案 |
...
範例案例：改 exit code / 改 stderr / 改 stdout → 本質都是「修改 Hook」→ 假選項。
...
衝突時用排序終結爭論：#4 vs #1 → #1 勝 → 工具維護建任務延後。
