...
### 實作者（parsley-flutter-developer / thyme-extension-engineer）
...
範例：
- [v0.17.0] tag-storage-adapter 有 8 個 catch 區塊，規格（observability-rules.md）要求「catch 區塊必須有日誌」。W2 測試未驗證任何 catch 區塊是否調用 logger，導致 P1-3（零日誌可觀測性）。防護：Phase 2 增加 mock logger 驗證 call count。
...
> 真實案例：cases/v0.17.0-test-blind-spots.md
...
| 測試資料工廠殘留舊版欄位 | 讀錯欄位名的 bug 碰巧通過（Q11，v0.17.0 P0-1） |
