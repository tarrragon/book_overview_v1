...
| SessionStart hook 觸發 | 是 | 待驗證（W3-028.2） | 待驗證（W3-028.2） |
| Stop hook 觸發 | 是 | 是 | 是 |
| handoff JSON 可讀 | 是 | 是 | 是（檔案系統層，session-agnostic） |
| ticket resume CLI 可用 | 是 | 是 | 是 |
| 自動 runqueue 提示出現 | 是（透過 session-start-scheduler-hint-hook） | 視 SessionStart 是否觸發 | 視 SessionStart 是否觸發 |

> **W3-028.2 追蹤**：SessionStart event 在 `/resume`（含前台與 bg）情境下的觸發行為待 0.19.0-W3-028.2 實機驗證。完成後本表將更新為確定狀態。
...
| 2. 重建全局視野 | 手動執行 `ticket track snapshot`（不依賴 SessionStart 自動提示） | 前台新 session 可依賴自動 hook 觸發；bg session resume 視 W3-028.2 結果而定 |
| 3. 查 runqueue 接手建議 | 手動執行 `ticket track runqueue --context=resume --top 3` | 同上 |
...
| 禁止 | 原因 |
|------|------|
| 假設 bg session resume 後 SessionStart hook 必定觸發 | 行為待 W3-028.2 驗證；目前應主動執行 snapshot / runqueue，不依賴自動提示 |
...
| 情境 | 處理 |
|------|------|
| W3-028.2 完成後行為確定 | 本章節「SessionStart 待驗證」欄位更新為確定狀態，並移除 W3-028.2 追蹤標記 |
...
- **W3-028.2 待落地**：實機驗證 SessionStart event 在 /resume bg session 的觸發行為
...
**Version**: 1.4.0 — 新增「/resume bg session 場景（v2.1.144+）」章節：前台 vs bg session resume 行為對比表、bg session resume 後 handoff 流程、SessionStart 觸發行為待驗證標記。對齊 W3-028 ANA 結論（bg session 直接衝擊低、handoff JSON session-agnostic、SessionStart 待 W3-028.2 實機驗證）
