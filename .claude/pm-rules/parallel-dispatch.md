...
### bgIsolation: none 並行安全警告

Claude Code v2.1.143+ 提供 `worktree.bgIsolation: "none"` 設定，讓 subagent 直接在主 repo working copy 操作（不建 worktree）。本設定對並行派發有額外風險，需特別注意。

**風險矩陣**：

| 風險類型 | bgIsolation: worktree（預設） | bgIsolation: none |
|---------|-----------------------------|------------------|
| Git index 競爭 | 各自隔離，安全 | **共享 index**，PC-092 風險必然化 |
| `.claude/` 並行 ≤ 2 規則 | 適用（worktree 內 `.claude/` 已禁編輯） | **未驗證**（單一 subagent 已驗證 success；3+ 並行待後續實驗） |
| 殭屍 worktree 累積 | 有，已有 GC hook | 無此問題 |
| 合併成本 | 每次需合併 | 無 |

**目前建議（v0.19.x，未驗證並行情境前）**：

- **不啟用 bgIsolation: none 全面切換**：並行 git index 競爭風險（PC-092）已知；無法量化效益
- **單一 subagent + `.claude/` 修改**：可考慮 bgIsolation: none（已驗證 success；無並行風險）；但 ARCH-015 主 repo cwd 規則仍適用，bgIsolation 不影響該決策
- **3+ 並行 `.claude/` 修改**：**禁止** — 維持 PC-137 並行 ≤ 2 規則
- 待並行受控實驗（0.19.0-W3-034.4）結論出來後評估全面策略 <!-- PC-093-exempt: ticket-tracked:W3-034.4 為策略升級的 trigger ticket -->
...
對應防護：派發 prompt 必含精準 git staging（禁 `git add .` / `git add -A`），見本文件「派發 prompt 必含精準 git staging」章節。

**對照 PC-137 警告**：

PC-137 規則「`.claude/` 並行 ≤ 2」是 worktree 模式下的觀察結論（W17-097.1-.4 + W17-174.2.1/.3/.4）。bgIsolation: none 下並行行為**未經實驗驗證**，**不可外推假設** PC-137 規則在 none 模式下仍成立或解除。在驗證前，並行 ≤ 2 規則對 bgIsolation: none 模式同樣適用。 <!-- PC-093-exempt: history:0.19.0-W3-034.1 W3-034.4 為實驗驗證歷史錨點 -->
