...
> **來源**：W2-005 四次代理人派發失敗，根因為寫入量（276 行）超出代理人單次任務的 context 預算。

| 寫入量 | 策略 |
|--------|------|
| < 50 行 | 單一代理人完成（含探索） |
| 50-150 行 | 單一代理人，prompt 含精確 Edit 指令或 Ticket 路徑 |
| > 150 行 | **必須拆分**：設計子任務（產出程式碼到 Ticket）+ 注入子任務（從 Ticket 讀取並寫入） |

**判斷方式**：PM 在派發前，根據設計文件或規格估算產出程式碼行數。不確定時，偏向拆分。
...
**Version**: 4.3.0 - 新增代理人寫入量閾值規則（0.17.2-W2-020，基於 W2-019 分析）
