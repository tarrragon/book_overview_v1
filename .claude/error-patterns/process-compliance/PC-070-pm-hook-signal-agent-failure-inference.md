...
2. **跳過 pm-role.md Step 0.5**：失敗判斷前置步驟明寫「對懷疑失敗的 agentId 呼叫 TaskOutput(task_id, block=false, timeout=3000)，只讀 `<status>` 標籤」，但 PM 直接跳到結論
