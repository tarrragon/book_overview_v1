...
1. 確認失敗類型
    +-- 回合耗盡 → 簡化 prompt，前台重派
    +-- 修改了錯誤檔案 → 調整 prompt 指定檔案，前台重派
    +-- 完全沒改 → 檢查 prompt 是否清楚，前台重派
    +-- 改壞了既有測試 → 回退變更，調整 prompt，前台重派
    |
    v
2. 永遠不自己寫（連「幫忙修一小段」都不行）
    |
    v
3. 如果連續 2 次派發失敗 → 建立 incident Ticket，分析是任務太複雜還是 prompt 品質問題
```
