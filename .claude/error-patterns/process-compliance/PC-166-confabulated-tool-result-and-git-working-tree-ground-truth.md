...
防護 D 原針對「別的 worktree / branch / 已刪分支」。W1-039 鑑識證實「別處」可以是**同 session 同 prompt 的另一個執行體**（ghost branch）——一個 prompt 觸發兩個並發主迴圈，輸家分支的 transcript 不存在（記錄 at-most-once），但其 commit / spawn / complete 是真實副作用（世界 at-least-once）。本執行體對此零記憶，第一反應常是腦補「並行 session」或「自己幻覺」，兩者都錯。
...
| 檔案 birth time | `stat -f %SB <file>`（macOS）機械節奏（固定秒距批量誕生） | 外部執行體痕跡 |
...
| 無任何痕跡 | 不可判（純 confabulation 或執行流被 runtime 棄置；CC 不導出棄流 trace 無法證偽） | 回防護 A-E 當 confabulation 處置 |

**宿主機制未定**（誠實邊界）：ghost branch 的觸發機制（通知重複投遞 / turn fork / runtime resume / 外部 agent SDK）尚未收斂，補證須回事發 transcript 環境（屬後續實驗）。但鑑識判據（有痕跡 vs 不可判）不依賴宿主機制裁決，可即時使用。
