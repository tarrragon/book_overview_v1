...
每個寫入操作（complete / commit / set-* / append-log / create / claim）後，用**一個與該操作無關的命令**查當前真實狀態，比對而非沿用上一個工具的回傳敘事。
...
批次同型操作（對 N 個目標做同一動作）後，逐項檢視每個回傳，特別注意 CLI error 行（如 `error: the following arguments are required`）。單一參數錯誤可能讓整批失敗，但敘事慣性會將其概括為「整批成功」。
