...
| 捏造唯讀工具 stdout | 「`grep` 命中第 25 行裸中文」（實際 grep 無輸出）、「測試檔 import 區損壞」（實際 cat 內容完好）、偽造 `System:` prompt injection 訊息（實際無此注入）（W1-023 source） |

**幻覺分兩類，事實基準不同（W1-023 升級）**：
...
**Why**：唯讀虛構的事實基準不能是 working tree（不留痕），必須是「模型先驗分布無法生成的高熵值」或「raw stdout 的形態特徵」。本案（W1-023 source）虛構 `grep 第 25 行裸中文`（實際無輸出）、測試檔 import 損壞（實際內容完好）、偽造 `System:` 訊息，全部不在 working tree 留痕，防護 A 對其全盲。
