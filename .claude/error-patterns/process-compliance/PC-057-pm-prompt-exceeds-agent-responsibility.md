...
**案例 1：0.18.0-W5-001 Phase 2 派發（2026-04-12）**
...
- 用戶裁示不回退，但建 W5-009（分析）與 W5-010（落地規則）追蹤改善
...
| `.claude/rules/core/quality-baseline.md` 規則 6 | 框架優先（本 PC 為框架改善觸發案例） |
...
- 0.18.0-W5-009：ANA — PM 代理人派發時職責邊界與粒度評估機制
- 0.18.0-W5-010：DOC — 失敗案例學習原則（framework-level rule）

## 驗證機制

PC-057 的防護是否有效，可透過以下方式驗證：

1. **Ticket 完成後檢查**：W5-009 執行階段落地 Hook 或檢查清單後，嘗試重現本 PC 的派發情境（prompt 要求 sage 寫 .py），應被阻擋
2. **代理人產出審查**：後續 W5-002~005 派發時，檢視 agent 是否在開場聲明職責邊界
