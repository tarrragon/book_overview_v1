...
if [ -d "node_modules" ]; then
    ...
    log "❌ node_modules 不存在，測試可能失敗"
    log "💡 建議執行: npm install --legacy-peer-deps"
fi
...
INTEGRATION_TEST_COUNT=$(find tests/integration -name "*.test.js" 2>/dev/null | wc -l)
...
TEST_CHANGES=$(git status --porcelain | grep -E "test\.js$|spec\.js$" | wc -l)
if [ "$TEST_CHANGES" -gt 0 ]; then
    log "⚠️  發現 $TEST_CHANGES 個未提交的測試檔案變更"
    git status --porcelain | grep -E "test\.js$|spec\.js$" | while read status file; do
        log "  $status $file"
    done
fi
