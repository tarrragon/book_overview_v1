...
## sync-pull Breaking Change Consumer Checklist

上游框架發生 breaking change（如目錄搬移、模組重命名）後，consumer 執行 sync-pull 應依此清單逐步驗證。來源：framework issue #10（v2.0.0 hooks/lib 合併）三個 consumer 實測經驗。

### 前置確認

- [ ] 確認 sync-pull 腳本本身可執行（bootstrap 問題：腳本自身 import 可能已斷裂，需先手動修正）
- [ ] 確認 `.claude/.sync-state.json` 含 `last_synced_base_sha`（缺少時 sync-pull 退化為 full overlay）
- [ ] 備份：`git stash` 或確認 working tree 乾淨

### sync-pull 後驗證

- [ ] 執行 `bash .claude/scripts/test-hook-imports.sh`，確認 0 failures
- [ ] 若有 failure：檢查 sys.path 是否指向正確目錄（搬移後舊路徑失效）
- [ ] 檢查殘留目錄：`ls .claude/hooks/lib/` 若有 `__init__.py` 殘留需刪除（Python 會優先解析為 package）
- [ ] 檢查 lib/ 內部 import：`rg 'from hook_utils' .claude/lib/` 若有命中需改為相對 import

### 非標準模式修復

- [ ] 掃描非標準 sys.path：`rg 'sys.path' .claude/hooks/ .claude/skills/*/hooks/ --no-heading | grep -v 'parents\[3\]'`
- [ ] 掃描未定義變數：`rg '_FRAMEWORK_HOOKS|_hooks_dir|_CLAUDE_DIR' .claude/ -g "*.py" --no-heading`
- [ ] 確認修復後再跑一次 `test-hook-imports.sh`

### 文字殘留清理（非阻擋，建議處理）

- [ ] `rg '<舊路徑>' .claude/ -g "*.py" --no-heading | grep -v error-patterns`
- [ ] `rg '<舊路徑>' .claude/ -g "*.md" -g "*.yaml" --no-heading | grep -v error-patterns | grep -v CHANGELOG`
- [ ] 測試檔案中的 fixture 路徑是否仍引用舊位置（影響測試正確性）
