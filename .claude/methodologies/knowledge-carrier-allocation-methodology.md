...
| `~/.claude/CLAUDE.md` + @import（user-level，CC 原生） | 本機所有專案 | 每回合自動 | 個人操作偏好、本機環境事實（跨專案但僅限本機） | 框架規範與錯誤學習（→ `.claude/`；user-level 不隨 repo 傳播、無 git 稽核、無編號去重，協作者與新機器拿不到） |
...
**Last Updated**: 2026-07-05 — memory 載體條目更新為捕獲時分流語意（規則 7 前移同步）；補 user-level（`~/.claude/CLAUDE.md` @import）載體條目（原地圖對機器層無條目的系統性盲區）
