...
### Session 實例：W10-035 Meta ANA 任務鏈（Dogfooding）

下列任務鏈展示三種移動方向的協同運作：

```
W5-002 (IMP, Phase 1 規格)
    ↓ 上移衍生（Phase 1 PM 驗收暴露缺口）
W10-034 (ANA, Phase 1 規格完整性)
    ↓ 上移衍生（單點分析揭示結構性問題）
W10-035 (ANA, Meta 摩擦力倒置)
    ├─ W10-035.1 (ANA, Proposal 評估強制機制)
    ├─ W10-035.2 (DOC, friction v3.0 階段曲線)
    ├─ W10-035.3 (ANA, Phase 3b Hook 密度審計)
    ├─ W10-035.4 (IMP, SA 系統衝突檢查強化)
    └─ W10-035.5 (ANA, Hook 設計指引前期先行)
    ↓ 水平衍生（理念層橫向延伸）
W10-036 (DOC, 父子責任理念修正) ← 本方法論章節對應 Ticket
    ├─ W10-036.1 (DOC, 任務鏈核心哲學)         ← 本章節
    ├─ W10-036.2 (DOC, ticket-lifecycle 規則)
    ├─ W10-036.3 (DOC, skills/ticket references)
    ├─ W10-036.5 (DOC, 任務設計拆分實務指南)
    └─ W10-036.4 (IMP, acceptance-gate-hook block)
```

**展示要點**：
- **上移**：W5-002 → W10-034 → W10-035 每步都是子節點發現問題後建立上層 ANA
- **下移**：W10-035 衍生 5 個子 ANA/DOC/IMP，責任分派到子層
- **水平**：W10-036 透過 relatedTo 與 W10-035 產生橫向連結（理念修正源自 Meta ANA 過程）
- **父責任**：W10-036 本身作為父 DOC，其 complete 前置依賴 5 個子 Ticket 全部 completed——本方法論章節完成即是其責任履行的一部分
...
> **原則**：同父下的兄弟任務之間應**平行無依賴**。有依賴關係時，依賴者應下沉一層或重組為聚合父結構。見 error-pattern `ARCH-017: 兄弟任務隱藏依賴`。
...
| **串行** | 兄弟間有順序但不直接依賴 | 同檔案連續修改，需避免 git 衝突 |
...
Q2: 依賴方向是否單向（A 被 B/C/... 依賴，A 是 hub）？
    |
    +-- 是 → 將 hub 與被依賴者包為聚合父（如 W10-036 重組範式）
    |
    +-- 否（依賴關係複雜） → Q3
         |
         v
...
### Dogfooding 實例：W10-036 結構重組

W10-036「父子責任理念修正」最初設計為 5 平行兄弟（.1/.2/.3/.4/.5），但 .2/.3/.5 都 relatedTo .1 哲學章節，.4 blockedBy 其他 4 個——違反兄弟無依賴原則。重組過程：
...
要點：
- W10-036 兄弟層從 5 個依賴纏繞 → 2 個清晰（方法論 vs Hook 實作）
