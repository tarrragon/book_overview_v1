...
```bash
# 1. 執行版號同步檢查 (確保 package.json 與 CHANGELOG.md 同步)
...
**四階段自動化檢查**：
1. **版號同步檢查**: 確保 package.json 與 CHANGELOG.md 版號一致
2. **版本正確性檢查**: 確保使用正確的版本號 (以 CHANGELOG.md 為準)
3. **工作狀態分析**: 規則比對判斷當前工作的狀態和階段
4. **記錄完整性驗證**: 檢查今日記錄和文件同步狀態
...
**📋 版本管理重要提醒**：
- 🎯 **以 CHANGELOG.md 為版號標準**: 所有版號以 CHANGELOG.md 最新版本為準
- 🔄 **自動版號同步**: 系統會自動將 package.json 同步到 CHANGELOG.md 版號
- ⚠️ **避免版號錯判**: 已修正錯誤的 v1.0.0 標記為 v0.10.x 系列
- 📝 **完成工作標記**: 完成的工作必須有明確的總結和狀態標記
