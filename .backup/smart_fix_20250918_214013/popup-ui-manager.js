...
class PopupUIManager {
  ...
  cacheElements () {
    ...
    if (!doc) {
      ...
      Logger.warn('[PopupUIManager] No document available for element caching')
      return
    }
    try {
      const probe = doc.getElementById('status-container')
      Logger.debug('[PopupUIManager] cacheElements probe exists?', !!probe, 'bodyLen', (doc.body && doc.body.innerHTML && doc.body.innerHTML.length))
    } catch (_) {}
    ...
    Object.keys(this.elementConfig).forEach(category => {
      ...
      Object.keys(categoryConfig).forEach(elementKey => {
        ...
        if (element) {
          ...
          // 記錄缺失的元素但不中斷執行
          // eslint-disable-next-line no-console
          Logger.warn(`[PopupUIManager] Element not found: ${elementId}`)
        }
      })
    })
  ...
  _validateCriticalElements () {
    ...
    if (missingCritical.length > 0) {
      // eslint-disable-next-line no-console
      Logger.warn(`[PopupUIManager] Missing critical elements: ${missingCritical.join(', ')}`)
    }
  }
  ...
  updateErrorActions (actions) {
    ...
    actions.forEach(action => {
      ...
      if (buttonKey && this.elements[buttonKey]) {
        ...
        // eslint-disable-next-line no-console
        Logger.warn(`[PopupUIManager] Unknown error action: ${action}`)
      }
    })
  }
  ...
  showSuccess (message) {
    if (!message || typeof message !== 'string') {
      // eslint-disable-next-line no-console
      Logger.warn('[PopupUIManager] showSuccess: Invalid message provided')
      return
    }
  ...
  showLoading (message) {
    ...
    if (!this.elements.loadingOverlay) {
      // eslint-disable-next-line no-console
      Logger.warn('[PopupUIManager] showLoading: Loading overlay element not found')
      return
    }
  ...
  _showContainerWithMessage (containerKey, messageKey, message) {
    ...
    if (!container) {
      // eslint-disable-next-line no-console
      Logger.warn(`[PopupUIManager] Container element not found: ${containerKey}`)
      return
    }
  ...
  bindEvent (elementId, eventType, callback) {
    if (!elementId || !eventType || typeof callback !== 'function') {
      // eslint-disable-next-line no-console
      Logger.warn('[PopupUIManager] bindEvent: Invalid parameters provided')
      return false
    }
    ...
    if (!element) {
      // eslint-disable-next-line no-console
      Logger.warn(`[PopupUIManager] bindEvent: Element not found: ${elementId}`)
      return false
    }
  ...
  showDiagnostic (content) {
    if (!content || typeof content !== 'string') {
      // eslint-disable-next-line no-console
      Logger.warn('[PopupUIManager] showDiagnostic: Invalid content provided')
      return
    }
  ...
  handleStatusEvent (statusEvent) {
    ...
    switch (statusEvent.type) {
      ...
      default:
        // eslint-disable-next-line no-console
        Logger.warn(`[PopupUIManager] Unknown status event type: ${statusEvent.type}`)
    }
  }
  ...
  updateStatusMessage (message) {
    if (!message || typeof message !== 'string') {
      // eslint-disable-next-line no-console
      Logger.warn('[PopupUIManager] updateStatusMessage: Invalid message provided')
      return
    }
