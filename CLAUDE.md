...
| `src/core/ui/components/ui-factory.js` | 全 Extension 共用 UI 元件工廠（無狀態純函式 createButton / createCard / createStatusIndicator / createProgressSection / createResultsSection / createErrorSection；class 名稱對齊 `src/core/design-system/component-rules.js`，不新增 CSS） |
...
| v0.17.x | Tag-based Book Model 重構（PROP-007，含 Schema/Storage/匯出/搜尋適配） | 已完成 v0.17.4 |
| v0.18.x | 測試重寫與品質恢復（對齊新 model，失敗測試歸零） | 已完成 v0.18.0 |
| v0.19.x | 端到端驗證、打包、內測準備與流程規格化 | 已完成 v0.19.1 |
| v0.20.x | 中文圖書分類法 + Tag 管理功能 | 已完成 v0.20.0 |
| v1.0.0 | 內測版本（Readmoo 提取 + 匯出入 + 跨裝置同步 + 打包分發） | 已完成 |
| v1.1.0 ~ v1.4.x | QR 離線同步、Skill 上架規範、框架維護、監測收斂 | 已完成 v1.4.1 |
| v1.5.0 | 多書城 — 博客來 DOM 觀察 + 適配器 + 多書城共用架構 | 已完成 |
| v1.6.0 | 多書城 — Kobo 台灣站 DOM 觀察 + 適配器 | 已完成 |
| v1.6.1 | Kobo 多地區路徑支援（語系無關選擇器 + 日本站路徑辨識） | — |
...
| v1.11.0 | 多書城 — Audible 有聲書 DOM 觀察 + 適配器 | — |
| v2.0.0 | 多書城統一管理 + Chrome Web Store 上架 | — |
