...
7. **繁體中文**: 所有溝通和文件使用台灣繁體中文，技術術語遵循台灣慣例
...
**❌ 絕對禁止的誇飾用語**:
- 過度讚譽詞彙: 企業級、完美、卓越、優秀、驚艷
- 誇大成就用語: 突破性、革命性、劃時代
- 情緒化描述: 令人興奮、振奮人心
- 絕對化表述: 完全解決、徹底消除、百分百成功

**✅ 必須採用的務實記錄方式**:
- **功能性描述**: "實作了 5層驗證策略，支援配置檢查、事件轉換、功能驗證"
- **效果記錄**: "測試執行時間從 45秒 降低至 23秒，提升 49%"
- **問題處理**: "修正了 3個主要架構問題：循環依賴、錯誤處理不一致、測試覆蓋不足"
- **風險評估**: "識別風險：跨模組通訊延遲可能影響響應時間，需持續監控"

**📊 標準報告結構要求**:
- **技術實現記錄**: 具體功能列表、測試覆蓋率數字、程式碼量化指標
- **問題與解決方案**: 具體問題描述、嘗試方案、最終選擇理由
- **品質指標客觀評估**: 測試通過率、效能數據對比、架構債務狀況
...
### 實作策略設計：
基於測試工程師提供的測試案例，規劃以下實作策略：
- **整體架構決策**: 選擇的設計模式、程式碼結構、模組分工
- **技術選擇理由**: 採用的技術方案、函式庫選擇、實作方法
- **最小實作原則**: 只實作讓測試通過的最小程式碼的具體策略
- **漸進式開發計劃**: 分階段讓測試通過的具體順序和方法

### 詳細實作指引：

#### 第一階段實作指引：
- **目標測試群組**: [列出這輪要通過的測試]
- **核心程式碼範例**: [提供關鍵邏輯的具體程式碼片段]
...
### feat
- 新增JSON匯入錯誤檢查，匯入無效檔案時顯示明確錯誤訊息
- 新增書籍去重功能，避免同一本書重複顯示

### fix
- 修正匯入JSON檔案時程式當機問題
- 修正Overview頁面搜尋結果不正確問題

### change
- 移除複雜同步設定，改用簡單的匯出/匯入JSON檔案
- 書籍資料現在只能手動匯入，不再自動同步
```
...
**📚 文件責任區分合規要求**:
所有Agent輸出必須符合「📚 專案文件責任明確區分」章節標準：

- **Agent規劃文件**: 必須符合**工作日誌**標準
  - 詳細、分析性、教學性風格
  - 完整的思考過程和技術決策記錄
  - 為協作者提供充分的背景資訊

- **嚴格禁止事項**:
  - ❌ 不得產出使用者導向的CHANGELOG內容 
  - ❌ 不得混淆TODO.md開發規劃責任
  - ❌ 不得使用抽象或無法驗證的描述
...
**🎨 Phase 1: 功能設計階段 (lavender-interface-designer)**
- **輸入要求**: 使用者需求、功能概述、技術限制
- **處理職責**: 功能需求分析、邊界條件識別、API介面設計、驗收標準制定
- **輸出標準**: 建立`docs/work-logs/vX.X.X-feature-design.md`工作日誌
- **交接檢查點**:
  - [ ] 功能需求清楚且具體，無抽象描述
...
**🧪 Phase 2: 測試設計階段 (sage-test-architect)**  
- **輸入要求**: Phase 1的功能設計工作日誌
- **處理職責**: 測試策略規劃、測試案例設計、測試環境設置
...
**💻 Phase 3: 實作規劃階段 (pepper-test-implementer)**
- **輸入要求**: 包含測試設計的完整工作日誌
- **處理職責**: 實作策略規劃、權宜方案識別、技術債務記錄，提供主線程完整實作指引
...
- **整體架構決策**: 選擇的設計模式、程式碼結構、模組分工
- **技術選擇理由**: 採用的技術方案、函式庫選擇、實作方法
- **依賴注入規劃**: 如何整合到現有事件驅動架構中

### 詳細實作指引：
- **Step-by-step 實作步驟**: 具體的編碼順序和每步驟的目標
- **關鍵程式碼範例**: 核心邏輯的具體程式碼片段示範
- **API 介面實作**: 詳細的函數簽名、參數處理、回傳值設計
...
**🏗️ Phase 4: 重構階段 (cinnamon-refactor-owl)**
- **輸入要求**: 包含實作記錄的完整工作日誌
- **處理職責**: 執行「🧠 TDD 驅動重構方法論」完整流程
- **輸出標準**: 建立獨立的重構專用工作日誌
- **交接檢查點**:
  - [ ] 重構方法論Phase 1-3完整執行
  - [ ] 預期管理和驗證過程完整記錄
...
**每個Agent完成後的標準檢查**:
- [ ] **文件合規**: 輸出符合「📚 專案文件責任明確區分」標準
- [ ] **角色職責**: 嚴格按照對應TDD階段職責執行
- [ ] **交接準備**: 為下一階段Agent提供充分上下文
- [ ] **品質標準**: 達到工作日誌詳細、分析性、教學性要求

**跨Agent協作驗證**:
- [ ] **階段連貫性**: 每個階段輸出與前階段輸入完全對應
- [ ] **資訊完整性**: 重要決策和發現在各階段間正確傳遞
- [ ] **一致性檢查**: 各階段描述的功能和技術方案保持一致
- [ ] **可追蹤性**: 從功能設計到重構完成的完整過程可追蹤
