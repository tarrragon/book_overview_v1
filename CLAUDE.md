...
**1. 工作日誌強制更新檢查**:
- [ ] **工作日誌已建立**: 每個完成的功能都必須有對應的工作日誌
- [ ] **過程記錄完整**: 包含 Red-Green-Refactor 的完整思考過程
- [ ] **決策理由清楚**: 記錄所有重要技術決策和理由
- [ ] **問題解決過程**: 詳細記錄遇到的問題和解決方案

**2. TODO.md 狀態同步檢查**:
- [ ] **完成狀態更新**: 將 TODO 中對應項目標記為完成 [x]
- [ ] **進度反映準確**: 確保 TODO 中的進度描述與實際一致
- [ ] **移除過時項目**: 刪除不再相關或已合併的工作項目
- [ ] **新增發現的工作**: 在開發過程中發現的新工作項目必須記錄
...
**必須更新的文件（按責任區分）**：
1. **`docs/todolist.md`** - 更新開發進度和狀態
   - 標記完成項目為 [x]
   - 更新當前專案狀態版本號
   - 調整下一步工作優先級

2. **`docs/work-logs/vX.X.X-work-log.md`** - 詳細記錄協作資訊
   - 完整的技術決策過程
   - 問題發現和解決方案分析
   - 為未來協作者提供的學習資訊

3. **`CHANGELOG.md`** - 使用者導向的版本更新說明
   - **僅記錄對使用者有價值的變更**
   - 使用簡潔語言描述功能改進
   - 每個項目不超過 3-5 行
