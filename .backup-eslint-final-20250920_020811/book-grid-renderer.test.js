  ...
  describe('🔧 錯誤處理', () => {
    ...
    test('應該處理事件總線錯誤', () => {
      mockEventBus.emit = jest.fn().mockImplementation(() => {
        throw (() => { const error = new Error('Event bus error'); error.code = ErrorCodes.GRID_EVENT_BUS_ERROR; error.details = { category: 'testing' }; return error })()
      })
    ...
    test('應該在 DOM 操作失敗時優雅降級', () => {
      mockContainer.appendChild = jest.fn().mockImplementation(() => {
        throw (() => { const error = new Error('DOM error'); error.code = ErrorCodes.GRID_DOM_OPERATION_ERROR; error.details = { category: 'testing' }; return error })()
      })
