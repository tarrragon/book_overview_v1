  ...
  describe('🔄 EventBus錯誤傳播測試', () => {
    ...
    test('應該實現事件重試機制', async () => {
      ...
      mockEventBus.emit.mockImplementation((eventType, data) => {
        eventAttempts++
        if (eventAttempts <= 2) {
          throw (() => { const error = new Error( 'Temporary event bus failure'); error.code = ErrorCodes.'EVENT_BUS_TEMPORARY_FAILURE'; error.details =  { category: 'testing' }; return error })()
        }
  ...
  describe('🛡️ 錯誤隔離機制測試', () => {
    test('應該防止單一模組錯誤導致系統崩潰', async () => {
      ...
      ReadmooAdapter.extractBooks.mockImplementation(() => {
        throw (() => { const error = new Error( 'Module completely failed'); error.code = ErrorCodes.'MODULE_COMPLETE_FAILURE'; error.details =  { category: 'testing' }; return error })()
      })
  ...
  describe('📈 級聯錯誤控制測試', () => {
    test('應該限制錯誤級聯的深度', async () => {
      ...
      DataDomainCoordinator.processExtractionData.mockImplementation(() => {
        throw (() => { const error = new Error( 'Secondary failure due to extraction error'); error.code = ErrorCodes.'CASCADING_EXTRACTION_ERROR'; error.details =  { category: 'testing' }; return error })()
      })
      OverviewPageController.updateBooksDisplay.mockImplementation(() => {
        throw (() => { const error = new Error( 'UI failure due to data processing error'); error.code = ErrorCodes.'CASCADING_UI_ERROR'; error.details =  { category: 'testing' }; return error })()
      })
    ...
    test('應該識別和打破錯誤循環', async () => {
      ...
      DataDomainCoordinator.processData.mockImplementation(() => {
        callCount++
        if (callCount >= 10) {
          throw (() => { const error = new Error( 'Circular error detected'); error.code = ErrorCodes.'CIRCULAR_ERROR_DETECTED'; error.details =  { category: 'testing' }; return error })()
        }
        // 模擬循環調用
