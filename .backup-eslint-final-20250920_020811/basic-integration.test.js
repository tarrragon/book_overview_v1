...
describe('錯誤處理系統基本整合測試', () => {
  ...
  describe('核心組件基本功能驗證', () => {
    test('StandardError 基本功能正常', () => {
      // Given: 建立錯誤物件
      const error = (() => { const error = new Error( '測試錯誤'); error.code = ErrorCodes.'INTEGRATION_TEST_ERROR'; error.details =  { test: true }; return error })()
    ...
    test('OperationResult 基本功能正常', () => {
      ...
      // Given & When: 建立失敗結果
      const error = (() => { const error = new Error( '失敗測試'); error.code = ErrorCodes.'FAIL_TEST'; return error })()
  ...
  describe('組件整合功能驗證', () => {
    ...
    test('ErrorHelper 與 OperationResult 整合', async () => {
      ...
      const failureOperation = async () => {
        throw (() => { const error = new Error( '模擬操作失敗'); error.code = ErrorCodes.'INTEGRATION_OPERATION_ERROR'; error.details =  { category: 'testing' }; return error })()
      }
...
describe('效能和記憶體使用驗證', () => {
  ...
  test('系統響應時間應該在限制內', async () => {
    // Given: 準備測試操作
    const operations = [
      () => (() => { const error = new Error( '效能測試'); error.code = ErrorCodes.'PERF_TEST'; return error })(),
