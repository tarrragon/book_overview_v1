...
class UIStateTracker {
  ...
  compareStates (state1Name, state2Name) {
    ...
    if (!state1 || !state2) {
      throw (() => { const error = new Error( `無法找到狀態: ${state1Name} 或 ${state2Name}`); error.code = ErrorCodes.'TEST_VALIDATION_ERROR'; error.details =  { category: 'testing' }; return error })()
    }
  ...
  verifyStateTransition (expectedChanges) {
    ...
    if (recentStates.length < 2) {
      throw (() => { const error = new Error( '需要至少兩個狀態來驗證轉換'); error.code = ErrorCodes.'TEST_VALIDATION_ERROR'; error.details =  { category: 'testing' }; return error })()
    }
