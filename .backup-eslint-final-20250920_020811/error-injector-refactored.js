...
class ErrorInjector {
  ...
  _validateInjectionParameters (api, method, error) {
    if (!api || !method || !error) {
      throw (() => { const error = new Error('All parameters are required for error injection'); error.code = ErrorCodes.ERROR_INJECTION_NOT_ENABLED; error.details = { category: 'testing' }; return error })()
    }
  }
  ...
  _validateDomSelector (selector) {
    if (!selector || typeof selector !== 'string') {
      throw (() => { const error = new Error('Selector must be a non-empty string'); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
  }
  ...
  _validateProbability (probability) {
    if (typeof probability !== 'number' || probability < 0 || probability > 1) {
      throw (() => { const error = new Error('Probability must be a number between 0 and 1'); error.code = ErrorCodes.TEST_ERROR; error.details = { category: 'testing' }; return error })()
    }
  }
