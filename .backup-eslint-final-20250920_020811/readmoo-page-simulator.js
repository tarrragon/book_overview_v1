...
class ReadmooPageSimulator {
  ...
  createMockShelfPage (books = []) {
    try {
      ...
      throw (() => { const error = new Error(`Failed to create mock shelf page: ${error.message}`); error.code = ErrorCodes.'TEST_ERROR'; error.details = { category: 'testing' }; return error })()
    }
  }
  ...
  modifyPageContent (modification) {
    try {
      ...
      throw (() => { const error = new Error(`Failed to modify page content: ${error.message}`); error.code = ErrorCodes.'TEST_ERROR'; error.details = { category: 'testing' }; return error })()
    }
  }
