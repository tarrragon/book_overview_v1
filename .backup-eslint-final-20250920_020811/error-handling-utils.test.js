  ...
  describe('🔄 重試機制', () => {
    test('應該實作指數退避重試', async () => {
      ...
      const failingFunction = jest.fn(() => {
        attempts++
        if (attempts < 3) {
          throw (() => { const error = new Error( `Attempt ${attempts} failed`); error.code = ErrorCodes.'TEST_ERROR'; error.details =  { category: 'testing' }; return error })()
        }
        return 'success'
      })
    ...
    test('應該在超過重試次數後失敗', async () => {
      const alwaysFailingFunction = jest.fn(() => {
        throw (() => { const error = new Error( 'Always fails'); error.code = ErrorCodes.'TEST_ERROR'; error.details =  { category: 'testing' }; return error })()
      })
