...
class MemoryLeakDetector {
  ...
  recordOperationStart (operationName, context = {}) {
    if (!this.isMonitoring) {
      throw (() => { const error = new Error('記憶體監控未啟動'); error.code = ErrorCodes.'MEMORY_DETECTOR_ERROR'; error.details = { category: 'testing' }; return error })()
    }
  ...
  async recordOperationEnd (operationId) {
    ...
    if (!operation) {
      throw (() => { const error = new Error(`找不到操作ID: ${operationId}`); error.code = ErrorCodes.'MEMORY_DETECTOR_ERROR'; error.details = { category: 'testing' }; return error })()
    }
